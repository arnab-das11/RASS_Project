    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans pb-20">
        ...
        <div className="flex items-center gap-3">
          ...
          {userInfo?.profilePicture ? (
            <img
              ...
              alt={userInfo.name}
              className="w-10 h-10 rounded-xl object-cover border-2 border-indigo-550/50 shadow-lg"
              onError={(e) => {
            ...
            <div className="w-10 h-10 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-xl flex items-center justify-center text-white font-black shadow-lg uppercase">
              {userInfo?.name?.charAt(0) || "I"}
            </div>
          )}

          <div>
            <h1 className="text-xl font-bold text-slate-850 tracking-tight">Instructor Portal</h1>
            <p className="text-xs text-indigo-600 font-bold uppercase tracking-wider">Welcome back, {userInfo?.name}</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/')} className="p-2 text-slate-400 hover:text-indigo-600 transition cursor-pointer" title="Home"><Home size={20} /></button>
          <button onClick={handleLogout} className="p-2 text-red-650 hover:text-red-500 transition bg-red-50 hover:bg-red-100 border border-red-200/40 rounded-lg flex items-center gap-2 text-sm font-bold cursor-pointer"><LogOut size={16} /> Logout</button>
        </div>
      </nav>
      ...
      <div className="max-w-7xl mx-auto px-6 mt-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-6">
          <div>
            <h2 className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 via-indigo-500 to-violet-600 tracking-tight mb-2">My Arsenal</h2>
            <p className="text-slate-500 font-semibold">Manage your published and pending courses.</p>
          </div>
          <button onClick={() => navigate('/instructor-course')} className="px-8 py-3.5 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white rounded-xl font-bold transition-all shadow-md shadow-indigo-100 flex items-center gap-2 transform hover:-translate-y-1 cursor-pointer">
        ...
        {myCourses.length === 0 ? (
          <div className="bg-white rounded-3xl p-12 text-center border border-slate-200 shadow-xl">
            <BookOpen className="mx-auto w-16 h-16 text-indigo-550/40 mb-4" />
              ...
              <div key={course._id} className="bg-white rounded-3xl overflow-hidden border border-slate-200 hover:border-indigo-500/50 hover:shadow-2xl transition-all shadow-md group flex flex-col">
                ...
                <div className="p-6 flex-1 flex flex-col">
                  ...
                  <div className="flex gap-4 text-xs font-bold text-slate-500 mb-6">
                    <span className="flex items-center gap-1"><Clock size={14} className="text-indigo-550" /> {course.duration}h</span>
                    <span className="flex items-center gap-1"><DollarSign size={14} className="text-emerald-600" /> {course.price}</span>
                  </div>
                  ...
                  <div className="mt-auto flex justify-between items-center border-t border-slate-100 pt-5">
                    ...
                    <div className="flex gap-2">
                      {course.status === 'approved' && (
                        <button onClick={() => handleRequestDelete(course._id)} className="p-2 bg-red-50 text-red-650 hover:bg-red-605 hover:bg-red-600 hover:text-white rounded-lg transition border border-red-200/40 cursor-pointer" title="Request Deletion">
                          <Trash2 size={18} />
                        </button>
                      )}
                    </div>

...
const CourseDetailsPage = () => {
  ...
  // --- HELPER: Handle Downloads Safely ---
  const getDownloadLink = (url) => {
    if (!url) return "#";
    
    // 1. Check if it is a "Raw" file (PDF, DOC, etc.)
    // Cloudinary puts "/raw/" in the URL for these files.
    const isRaw = url.includes("/raw/");

    // 2. If it is an Image/Video, we CAN force download using flags
    if (url.includes("cloudinary.com") && url.includes("/upload/") && !isRaw) {
      const parts = url.split("/upload/");
      return `${parts[0]}/upload/fl_attachment/${parts[1]}`;
    }

    // 3. If it is a Raw file (PDF), return the ORIGINAL URL.
    // Adding flags to raw files breaks the link.
    return url;
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
            ...
            <div className="flex items-center gap-6 pt-4">
                 <div className="text-3xl font-bold text-white">
                    {course.price === 0 ? (
                        ...
                        <span className="flex items-center"><IndianRupee size={28}/> {course.price}</span>
                    )}
                 </div>
                 ...
                 {!isEnrolled ? (
                     <button 
                        ...
                        className="bg-green-500 hover:bg-green-600 text-white px-8 py-3 rounded-xl font-bold text-lg shadow-lg shadow-green-900/30 transition-transform transform hover:-translate-y-1 flex items-center gap-2"
                     >
                     ...
                     <button className="bg-blue-500/20 text-blue-200 px-8 py-3 rounded-xl font-bold text-lg border border-blue-400/30 cursor-default">
                        Already Enrolled
                     </button>
                 )}
            </div>
      ...
      <div className="flex-1 max-w-7xl mx-auto px-6 py-10 grid grid-cols-1 lg:grid-cols-3 gap-10 w-full">
        ...
        {/* RIGHT COLUMN (1/3): Curriculum List */}
                  ...
                  <div key={index} className="border-b border-gray-100 last:border-0">
                    ...
                    {expandedSections[index] && (
                      <div className="bg-white animate-fade-in">
                        ...
                        {/* --- RESOURCES (FIXED: Safe Download/Open) --- */}
                        {section.resources && section.resources.map((res, rIdx) => (
                          <a 
                             key={rIdx} 
                             // 1. Use the Helper (Make sure you updated the helper function at the top!)
                             href={getDownloadLink(res.url)} 
                             
                             // 2. CRITICAL: Open in new tab. 
                             // This allows PDFs to open (and then be downloaded) without crashing.
                             // DOCX files will just download automatically.
                             target="_blank" 
                          ...
                          >
                             ...
                             <div className="flex flex-col">
                               ...
                               <span className="text-[10px] text-gray-400 uppercase">
                                  ...
                                  {res.url.includes('.pdf') ? "Open PDF" : "Download File"}
                               </span>
                             </div>
                          </a>
                        ))}
                        ...
                        {section.links && section.links.map((link, lIdx) => (
                          ...
                          >
                             ...
                             <span className="text-sm text-gray-600 font-medium hover:text-purple-700 truncate">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            ...
            <ul className="space-y-4 text-gray-600 text-sm">
              <li className="flex justify-between items-center border-b border-gray-50 pb-3">
                <div className="flex items-center gap-2 text-gray-500"><Clock size={18} /> <span>Duration</span></div>
                <span className="font-semibold text-gray-800">{course.duration} Hours</span>
              </li>
              <li className="flex justify-between items-center border-b border-gray-50 pb-3">
                <div className="flex items-center gap-2 text-gray-500"><MonitorPlay size={18} /> <span>Lectures</span></div>
                <span className="font-semibold text-gray-800">{getTotalVideos()} Videos</span>
              </li>
              <li className="flex justify-between items-center border-b border-gray-50 pb-3">
                <div className="flex items-center gap-2 text-gray-500"><BarChart size={18} /> <span>Skill Level</span></div>
                <span className="font-semibold text-gray-800 capitalize">{course.level}</span>
              </li>
              <li className="flex justify-between items-center border-b border-gray-50 pb-3">
                <div className="flex items-center gap-2 text-gray-500"><Globe size={18} /> <span>Language</span></div>
                <span className="font-semibold text-gray-800">English</span>
              </li>
              <li className="flex justify-between items-center pt-1">
                <div className="flex items-center gap-2 text-gray-500"><Award size={18} /> <span>Certificate</span></div>
